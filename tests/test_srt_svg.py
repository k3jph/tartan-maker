...
ASYM = """
name: Example Asymmetrical Tartan
...
def test_sym_svg_renders_expanded_direct_repeat(tmp_path):
    spec = load_spec(write_tmp(tmp_path, SYM))
    svg = render_svg(spec)
    assert 'width="182" height="182"' in svg
    assert '<rect class="B" y="0" height="12"' in svg
    assert '<rect class="B" x="170" width="12" height="100%"/>' in svg
    assert 'B/24' not in svg
...
def test_svg_generates_explicit_warp_rects_not_rotated(tmp_path):
    spec = load_spec(write_tmp(tmp_path, ASYM))
    svg = render_svg(spec)
    assert 'id="weftThreadcounts"' in svg
    assert 'id="warpThreadcounts"' in svg
    assert 'transform="rotate' not in svg
    assert '<rect class="W" x="0" width="8" height="100%"/>' in svg
    assert '<rect class="LB" x="8" width="20" height="100%"/>' in svg
    assert '<!-- Generated by tartan-maker' in svg


def test_png_contains_generator_metadata_when_backend_available(tmp_path):
    from tartan_maker import render_png

    spec = load_spec(write_tmp(tmp_path, ASYM))
    png = render_png(spec)
    assert png.startswith(b"\x89PNG\r\n\x1a\n")
    assert b"Software\x00tartan-maker 0.9.0" in png
    assert b"Description\x00Generated by tartan-maker 0.9.0 from SVG for Example Asymmetrical Tartan" in png
